...
class ImportEnforcer:
    ...
    def _process_import_node(
        self, graph: nx.DiGraph, module_name: str, node: ast.Import
    ) -> None:
    ...
    def _process_import_from_node(
        self, graph: nx.DiGraph, module_name: str, node: ast.ImportFrom
    ) -> None:
    ...
    def _group_imports(self, import_nodes: List[ast.AST]) -> Dict[str, List[ast.AST]]:
        ...
        groups = {"stdlib": [], "third_party": [], "local": [], "relative": []}
    ...
    def _check_alphabetical_order(
        self, import_groups: Dict[str, List[ast.AST]]
    ) -> List[Dict]:
