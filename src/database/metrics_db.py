...
This module provides functionality for storing, retrieving, and managing performance
metrics in a database, enabling long-term tracking and analysis of project performance.
"""
...
class MetricsDatabase:
    ...
    def __init__(self, db_path: Union[str, Path]):
        ...
        self.logger = logging.getLogger("ps2.database.metrics_db")
    ...
    def query_metrics(
        ...
        for row in cursor.fetchall():
            ...
            metric["context"] = (
                json.loads(metric["context"]) if metric["context"] else {}
            )
    ...
    def export_metrics(
        ...
        Args:
            ...
            tags: Export metrics with these tags. If None, don't filter by tags.
...
def connect_database(db_path: Union[str, Path]) -> MetricsDatabase:
    ...
    # If already connected to this database, return the existing connection
    if _db_connection and _db_path == str(db_path):
        return _db_connection

    # Close the existing connection if connecting to a different database
    if _db_connection and _db_path != str(db_path):
...
def store_metric(
    ...
    if db_path or not _db_connection:
        db_path = db_path or os.environ.get("PS2_METRICS_DB", "ps2_metrics.db")
        _db_connection = connect_database(db_path)
...
def export_metrics(
    ...
    # Export the metrics
    return _db_connection.export_metrics(
        output_file, format, name, start_time, end_time, tags
    )
